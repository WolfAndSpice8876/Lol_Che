...
class _SynergyPageState extends State<SynergyPage> {
  ...
  late List<String> origins = [ //final
    '전체'
  ];
  ...
  _SynergyPageState(){
    origins.addAll(DataLibrary.trait.originsByKoreanName.entries.map( (entry) => entry.key).toList());
    classes.addAll(DataLibrary.trait.classesByKoreanName.entries.map( (entry) => entry.key).toList());
  }
  ...
  List<ChampionDto> getChampionList(String? origin_ , String? class_){
    ...
    if(origin_ == null || origin_ == "전체"){
      DataLibrary.trait.findAllByName(class_!).members.forEach((element){
        result.add(DataLibrary.champion.findAllByName(element));
      });
      return result;
    }

    if(class_ == null || class_ == "전체"){
      DataLibrary.trait.findAllByName(origin_).members.forEach((element) {
        result.add(DataLibrary.champion.findAllByName(element));
      });
      return result;
    }

    List<String> originMembers = DataLibrary.trait.findAllByName(origin_).members;
    List<String> classMembers =  DataLibrary.trait.findAllByName(class_).members;

    originMembers.forEach((element0) {
      classMembers.forEach((element1) {
        if(element0 == element1){
          result.add(DataLibrary.champion.findAllByName(element1));
        }
      });
    });
          ...
          children: [
            ...
            Container(
              ...
              child: Row(
                ...
                children: [
                  Widget_DropDown("계열" , origins,originValue),
                  Widget_DropDown("직업" , classes,classValue),
                ],
              ),
            ),
